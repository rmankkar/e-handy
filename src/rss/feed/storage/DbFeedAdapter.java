...
public class DbFeedAdapter {
	...
	private static class DbHelper extends SQLiteOpenHelper {
		...
		DbHelper(DbFeedAdapter dbfa) {
			super(dbfa.mCtx, DbSchema.DATABASE_NAME, null,
					DbSchema.DATABASE_VERSION);
			mDbfa = dbfa;
		}
		...
		public void onCreate(SQLiteDatabase db) {
			...
			try {
				...
				// Show Startup Dialog on install
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			if (oldVersion <= 2) {
				...
				String alter_table = "ALTER TABLE "
						+ DbSchema.FeedSchema.TABLE_NAME + " ADD "
						+ DbSchema.FeedSchema.COLUMN_HOMEPAGE + " TEXT;";
				db.execSQL(alter_table);
				alter_table = "ALTER TABLE " + DbSchema.ItemSchema.TABLE_NAME
						+ " ADD " + DbSchema.ItemSchema.COLUMN_CONTENT
						+ " TEXT;";
				db.execSQL(alter_table);
				db.execSQL(DbSchema.EnclosureSchema.CREATE_TABLE);
				...
				feedCursor = db
						.query(DbSchema.FeedSchema.TABLE_NAME,
								new String[] { DbSchema.FeedSchema._ID }, null,
								null, null, null, DbSchema.FeedSchema._ID
										+ DbSchema.SORT_ASC);
				feedCursor.moveToFirst();
				while (!feedCursor.isAfterLast()) {
					feedId = feedCursor.getLong(feedCursor
							.getColumnIndex(DbSchema.FeedSchema._ID));
					itemCursor = db.query(DbSchema.ItemSchema.TABLE_NAME, null,
							DbSchema.ItemSchema.COLUMN_FEED_ID + "=?",
							new String[] { Long.toString(feedId) }, null, null,
							DbSchema.ItemSchema._ID + DbSchema.SORT_ASC);
					itemCursor.moveToFirst();

					while (!itemCursor.isAfterLast()) {
						itemId = itemCursor.getLong(itemCursor
								.getColumnIndex(DbSchema.ItemSchema._ID));
						if (!itemCursor
								.isNull(itemCursor
										.getColumnIndex(DbSchema.ItemSchema.COLUMN_DESCRIPTION)))
							itemDescription = itemCursor
									.getString(itemCursor
											.getColumnIndex(DbSchema.ItemSchema.COLUMN_DESCRIPTION));
						if (itemDescription != null) {
							SpannableStringBuilder spannedStr = (SpannableStringBuilder) Html
									.fromHtml(itemDescription.toString().trim());
							Object[] spannedObjects = spannedStr.getSpans(0,
									spannedStr.length(), Object.class);
							for (int i = 0; i < spannedObjects.length; i++) {
								if (spannedObjects[i] instanceof ImageSpan)
									spannedStr
											.replace(
													spannedStr
															.getSpanStart(spannedObjects[i]),
													spannedStr
															.getSpanEnd(spannedObjects[i]),
													"");
							}

							itemDescription = spannedStr.toString().trim()
									+ System.getProperty("line.separator");
							itemContent = spannedStr.toString().trim()
									+ System.getProperty("line.separator");

							values = new ContentValues();
							values.put(DbSchema.ItemSchema.COLUMN_DESCRIPTION,
									itemDescription);
							values.put(DbSchema.ItemSchema.COLUMN_CONTENT,
									itemContent);

							db.update(DbSchema.ItemSchema.TABLE_NAME, values,
									DbSchema.ItemSchema._ID + "=?",
		...
		private List<Feed> getOPMLResourceFeeds()
				throws XmlPullParserException, MalformedURLException,
				IOException {
			List<Feed> feeds = new ArrayList<Feed>();
			Feed feed;

			XmlResourceParser parser = mDbfa.mCtx.getResources().getXml(
					R.xml.feeds);
			...
			while (eventType != XmlResourceParser.END_DOCUMENT) {
				if (eventType == XmlResourceParser.START_TAG) {
					...
					if (tagName.equals("outline")
						...
						feed.setURL(new URL(parser.getAttributeValue(null,
								"xmlUrl")));
				...
				while (feedsIterator.hasNext()) {
					...
					if (!populated)
						Log.e(LOG_TAG,
								"Feed with title '"
										+ feed.getTitle()
										+ "' cannot be populated into the database. Feed URL: "
		...
		private boolean updateFeed(SQLiteDatabase db, Feed feed) {
			...
			ContentValues values = mDbfa.getContentValues(feed);
			updated = (db.update(DbSchema.FeedSchema.TABLE_NAME, values,
					DbSchema.FeedSchema._ID + "=?",
					new String[] { Long.toString(feed.getId()) }) > 0);
			return updated;
		}

		private long hasFeed(SQLiteDatabase db, Feed feed) {
			long feedId = -1;
			Cursor cursor = db.query(DbSchema.FeedSchema.TABLE_NAME, null,
					DbSchema.FeedSchema.COLUMN_URL + "=?", new String[] { feed
							.getURL().toString() }, null, null, null);
	...
	public void cleanDbItems(long feedId) {
		...
		cleanNbr = items.size() - 20;
		if (cleanNbr > 0) {
			itemListIterator = items.listIterator();
			while (itemListIterator.hasNext()
					&& (itemListIterator.nextIndex() > cleanNbr)) {
	...
	public List<Feed> getFeeds() {
		List<Feed> feeds = new ArrayList<Feed>();
		Cursor cursor = mDb.query(DbSchema.FeedSchema.TABLE_NAME,
				new String[] { DbSchema.FeedSchema._ID }, null, null, null,
				null, DbSchema.FeedSchema._ID + DbSchema.SORT_ASC);
		cursor.moveToFirst();
			...
			if (cursor.moveToFirst()) {
				...
				while (!cursor.isAfterLast()) {
					...
					feed.setURL(new URL(cursor.getString(cursor
							.getColumnIndex(DbSchema.FeedSchema.COLUMN_URL))));
					feed.setHomePage(new URL(
							cursor.getString(cursor
									.getColumnIndex(DbSchema.FeedSchema.COLUMN_HOMEPAGE))));
					feed.setTitle(cursor.getString(cursor
							.getColumnIndex(DbSchema.FeedSchema.COLUMN_TITLE)));
					if (!cursor.isNull(cursor
							.getColumnIndex(DbSchema.FeedSchema.COLUMN_TYPE)))
						feed.setType(cursor.getString(cursor
								.getColumnIndex(DbSchema.FeedSchema.COLUMN_TYPE)));
					if (!cursor
							.isNull(cursor
									.getColumnIndex(DbSchema.FeedSchema.COLUMN_REFRESH)))
						feed.setRefresh(new Date(
								cursor.getLong(cursor
										.getColumnIndex(DbSchema.FeedSchema.COLUMN_REFRESH))));
					feed.setEnabled(cursor.getInt(cursor
							.getColumnIndex(DbSchema.FeedSchema.COLUMN_ENABLE)));
					feed.setItems(getItems(id, 1, -1));
					cursor.moveToNext();
				}
			} else {
				throw new FeedException("Feed with id " + id
	...
	public long addFeed(ContentValues values, List<Item> items) {
		...
		if (feedId == -1)
			Log.e(LOG_TAG,
					"Feed '"
							+ values.getAsString(DbSchema.ItemSchema.COLUMN_TITLE)
							+ "' could not be inserted into the database. Feed values: "
							+ values.toString());
	...
	public boolean updateFeed(long id, ContentValues values, List<Item> items) {
		boolean feedUpdated = (mDb.update(DbSchema.FeedSchema.TABLE_NAME,
				values, DbSchema.FeedSchema._ID + "=?",
	...
	public boolean hasItem(long feedId, Item item) {
		Cursor cursor = mDb.query(
				DbSchema.ItemSchema.TABLE_NAME,
				null,
				DbSchema.ItemSchema.COLUMN_FEED_ID + "=? AND ("
						+ DbSchema.ItemSchema.COLUMN_LINK + "=? OR "
						+ DbSchema.ItemSchema.COLUMN_GUID + "=? OR "
						+ DbSchema.ItemSchema.COLUMN_TITLE + "=?)",
				new String[] { Long.toString(feedId),
						item.getLink().toString(), item.getGuid(),
						item.getTitle() }, null, null, null);
	...
	public boolean removeFeed(long id) {
		removeItems(id);
		return (mDb.delete(DbSchema.FeedSchema.TABLE_NAME,
				DbSchema.FeedSchema._ID + "=?",
				new String[] { Long.toString(id) }) > 0);
	}
	...
	public long getItemFeedId(long itemId) {
		...
		Cursor cursor = null;
		try {
			cursor = mDb.query(DbSchema.ItemSchema.TABLE_NAME, null,
					DbSchema.ItemSchema._ID + "=?",
			...
			if (cursor.moveToFirst()) {
				while (!cursor.isAfterLast()) {
					feedId = cursor
							.getLong(cursor
									.getColumnIndex(DbSchema.ItemSchema.COLUMN_FEED_ID));
					cursor.moveToNext();
				}
			} else {
				throw new FeedException("Feed id for item id " + itemId
						+ " not found in the database.");
			}
		} catch (FeedException fe) {
			Log.e(LOG_TAG, "", fe);
			...
			if (cursor.moveToFirst()) {
				...
				while (!cursor.isAfterLast()) {
					...
					item.setLink(new URL(cursor.getString(cursor
							.getColumnIndex(DbSchema.ItemSchema.COLUMN_LINK))));
					item.setGuid(cursor.getString(cursor
							.getColumnIndex(DbSchema.ItemSchema.COLUMN_GUID)));
					item.setTitle(cursor.getString(cursor
							.getColumnIndex(DbSchema.ItemSchema.COLUMN_TITLE)));
					if (!cursor
							.isNull(cursor
									.getColumnIndex(DbSchema.ItemSchema.COLUMN_DESCRIPTION)))
					...
					if (!cursor
							.isNull(cursor
									.getColumnIndex(DbSchema.ItemSchema.COLUMN_CONTENT)))
						item.setContent(cursor.getString(cursor
								.getColumnIndex(DbSchema.ItemSchema.COLUMN_CONTENT)));
					if (!cursor.isNull(cursor
							.getColumnIndex(DbSchema.ItemSchema.COLUMN_IMAGE)))
						item.setImage(new URL(
								cursor.getString(cursor
										.getColumnIndex(DbSchema.ItemSchema.COLUMN_IMAGE))));
					item.setPubdate(new Date(
							cursor.getLong(cursor
									.getColumnIndex(DbSchema.ItemSchema.COLUMN_PUBDATE))));
					item.setFavorite(cursor.getInt(cursor
							.getColumnIndex(DbSchema.ItemSchema.COLUMN_FAVORITE)));
					item.setRead(cursor.getInt(cursor
							.getColumnIndex(DbSchema.ItemSchema.COLUMN_READ)));
					item.setEnclosures(getEnclosures(id, 1, -1));
					cursor.moveToNext();
				}
			} else {
				throw new FeedException("Item with id " + id
						+ " not found in the database.");
			}
		} catch (FeedException fe) {
			Log.e(LOG_TAG, "", fe);
	...
	public Item getFirstItem(long feedId) {
		Cursor cursor = mDb.query(DbSchema.ItemSchema.TABLE_NAME, null,
				DbSchema.ItemSchema.COLUMN_FEED_ID + "=?",
				new String[] { Long.toString(feedId) }, null, null,
				DbSchema.ItemSchema._ID + DbSchema.SORT_ASC, "1");
		boolean hasItem = cursor.moveToFirst();
		Item firstItem = null;
		if (hasItem)
			firstItem = getItem(cursor.getLong(cursor
					.getColumnIndex(DbSchema.ItemSchema._ID)));
	...
	public Item getLastItem(long feedId) {
		Cursor cursor = mDb.query(DbSchema.ItemSchema.TABLE_NAME, null,
				DbSchema.ItemSchema.COLUMN_FEED_ID + "=?",
				new String[] { Long.toString(feedId) }, null, null,
				DbSchema.ItemSchema._ID + DbSchema.SORT_DESC, "1");
		boolean hasItem = cursor.moveToFirst();
		Item lastItem = null;
		if (hasItem)
			lastItem = getItem(cursor.getLong(cursor
					.getColumnIndex(DbSchema.ItemSchema._ID)));
	...
	public long getNextItemId(long feedId, long currentItemId) {
		...
		try {
			...
			if (cursor.moveToFirst()) {
				...
				throw new FeedException("Feed id " + feedId
						+ " not found in the database.");
			}
		} catch (FeedException fe) {
	...
	public List<Item> getItems(long feedId, int sort, int maxItems) {
		...
		if (maxItems > 0)
			cursor = mDb.query(DbSchema.ItemSchema.TABLE_NAME, null,
					DbSchema.ItemSchema.COLUMN_FEED_ID + "=?",
					new String[] { Long.toString(feedId) }, null, null,
					DbSchema.ItemSchema._ID + DbSchema.ORDERS[sort],
					Integer.toString(maxItems));
		else
			cursor = mDb.query(DbSchema.ItemSchema.TABLE_NAME, null,
					DbSchema.ItemSchema.COLUMN_FEED_ID + "=?",
					new String[] { Long.toString(feedId) }, null, null,
		...
		cursor.moveToFirst();
		while (!cursor.isAfterLast()) {
			Item item = getItem(cursor.getLong(cursor
					.getColumnIndex(DbSchema.ItemSchema._ID)));
			if (item != null)
				items.add(item);
			cursor.moveToNext();
	...
	public List<Item> getNotFavoriteItems(long feedId, int maxItems) {
		...
		if (maxItems > 0)
			cursor = mDb.query(
					...
					new String[] { Long.toString(feedId),
							Integer.toString(DbSchema.OFF) }, null, null,
					DbSchema.ItemSchema._ID + DbSchema.SORT_ASC,
					Integer.toString(maxItems));
		else
			cursor = mDb.query(
	...
	public List<Item> getFavoriteItems(int maxItems) {
		...
		if (maxItems > 0)
			cursor = mDb.query(DbSchema.ItemSchema.TABLE_NAME, null,
					DbSchema.ItemSchema.COLUMN_FAVORITE + " =?",
					new String[] { Integer.toString(DbSchema.ON) }, null, null,
					DbSchema.ItemSchema._ID + DbSchema.SORT_DESC,
					Integer.toString(maxItems));
		else
			cursor = mDb.query(DbSchema.ItemSchema.TABLE_NAME, null,
					DbSchema.ItemSchema.COLUMN_FAVORITE + " =?",
		...
		if (itemId == -1)
			Log.e(LOG_TAG,
					"Item '"
							+ values.getAsString(DbSchema.ItemSchema.COLUMN_TITLE)
							+ "' for Feed id "
							+ values.getAsLong(DbSchema.ItemSchema.COLUMN_FEED_ID)
							+ " could not be inserted into the database. Item values: "
							+ values.toString());
	...
	public boolean updateItem(long id, ContentValues values,
			List<Enclosure> enclosures) {
		boolean itemUpdated = (mDb.update(DbSchema.ItemSchema.TABLE_NAME,
				values, DbSchema.ItemSchema._ID + "=?",
				new String[] { Long.toString(id) }) > 0);
		...
		if (itemUpdated && enclosures != null) {
			...
			while (iterator.hasNext()) {
				...
				updateEnclosure(values.getAsLong(DbSchema.ItemSchema._ID),
						enclosure);
			}
		}
	...
	public boolean removeItem(long id) {
		removeEnclosures(id);
		return (mDb.delete(DbSchema.ItemSchema.TABLE_NAME,
				DbSchema.ItemSchema._ID + "=?",
				new String[] { Long.toString(id) }) > 0);
	}
	...
	public List<Enclosure> getEnclosures(long itemId, int sort, int maxItems) {
		...
		Cursor cursor = null;
		if (maxItems > 0)
			cursor = mDb.query(DbSchema.EnclosureSchema.TABLE_NAME, null,
					DbSchema.EnclosureSchema.COLUMN_ITEM_ID + "=?",
					new String[] { Long.toString(itemId) }, null, null,
					DbSchema.EnclosureSchema._ID + DbSchema.ORDERS[sort],
					Integer.toString(maxItems));
		else
			cursor = mDb.query(DbSchema.EnclosureSchema.TABLE_NAME, null,
					DbSchema.EnclosureSchema.COLUMN_ITEM_ID + "=?",
		...
		cursor.moveToFirst();
		while (!cursor.isAfterLast()) {
			Enclosure enclosure = getEnclosure(cursor.getLong(cursor
					.getColumnIndex(DbSchema.EnclosureSchema._ID)));
			if (enclosure != null)
	...
	public Enclosure getEnclosure(long id) {
		...
		try {
			cursor = mDb.query(DbSchema.EnclosureSchema.TABLE_NAME, null,
					DbSchema.EnclosureSchema._ID + "=?",
					new String[] { Long.toString(id) }, null, null, null);
			if (cursor.moveToFirst()) {
				enclosure = new Enclosure();
				while (!cursor.isAfterLast()) {
					enclosure.setId(cursor.getLong(cursor
							.getColumnIndex(DbSchema.EnclosureSchema._ID)));
					enclosure
							.setMime(cursor.getString(cursor
									.getColumnIndex(DbSchema.EnclosureSchema.COLUMN_MIME)));
					enclosure
							.setURL(new URL(
									cursor.getString(cursor
											.getColumnIndex(DbSchema.EnclosureSchema.COLUMN_URL))));
					cursor.moveToNext();
				}
			} else {
				throw new FeedException("Enclosure with id " + id
						+ " not found in the database.");
			}
		} catch (FeedException fe) {
			Log.e(LOG_TAG, "", fe);
	...
	public ContentValues getContentValues(long itemId, Enclosure enclosure) {
		...
		values.put(DbSchema.EnclosureSchema.COLUMN_URL, enclosure.getURL()
				.toString());
		return values;
	}
	...
	public long addEnclosure(long itemId, ContentValues values) {
		long enclosureId = mDb.insert(DbSchema.EnclosureSchema.TABLE_NAME,
				null, values);
		if (enclosureId == -1)
			Log.e(LOG_TAG,
					"Enclosure '"
							+ values.getAsString(DbSchema.EnclosureSchema.COLUMN_URL)
							+ "' for Item id "
							+ values.getAsLong(DbSchema.EnclosureSchema.COLUMN_ITEM_ID)
							+ " could not be inserted into the database. Enclosure values: "
							+ values.toString());
		return enclosureId;
	}

	public boolean updateEnclosure(long itemId, Enclosure enclosure) {
		return updateEnclosure(enclosure.getId(),
				getContentValues(itemId, enclosure));
	}

	public boolean updateEnclosure(long id, ContentValues values) {
		return (mDb.update(DbSchema.EnclosureSchema.TABLE_NAME, values,
				DbSchema.EnclosureSchema._ID + "=?",
				new String[] { Long.toString(id) }) > 0);
	}

	public boolean removeEnclosures(long itemId) {
		return (mDb.delete(DbSchema.EnclosureSchema.TABLE_NAME,
				DbSchema.EnclosureSchema.COLUMN_ITEM_ID + "=?",
